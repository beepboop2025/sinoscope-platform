import { memo, useState, type ReactElement, type FormEvent, type MouseEvent, type ChangeEvent, type CSSProperties } from 'react';
...
interface AlertConfigModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAdd: (config: AlertConfigData) => void;
}

const AlertConfigModal = memo(({ isOpen, onClose, onAdd }: AlertConfigModalProps): ReactElement | null => {
  ...
  return (
    <div onClick={onClose} style={{
      ...
      <div onClick={(e: MouseEvent) => e.stopPropagation()} style={{
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          <div>
            ...
            <input value={symbol} onChange={(e: ChangeEvent<HTMLInputElement>) => setSymbol(e.target.value.toUpperCase())} placeholder="AAPL, BTC, etc." style={inputStyle} autoFocus />
          </div>
          <div>
            <label style={{ fontSize: 10, color: 'var(--text-3)', marginBottom: 3, display: 'block' }}>Condition</label>
            <select value={condition} onChange={(e: ChangeEvent<HTMLSelectElement>) => setCondition(e.target.value)} style={inputStyle}>
          ...
          <div>
            ...
            <input value={threshold} onChange={(e: ChangeEvent<HTMLInputElement>) => setThreshold(e.target.value)} type="number" step="any" placeholder={condition.includes('pct') ? '5' : '150.00'} style={inputStyle} />
          </div>
