...
function getDateRange(offsetWeeks: number = 0): { from: string; to: string } {
  const now = new Date();
...
const PanelEarningsCalendar = memo((): ReactElement => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (data && (data as EarningsEntry[]).length > 0) {
        setEarnings(data as EarningsEntry[]);
      } else {
