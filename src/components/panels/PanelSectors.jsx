...
function getHeatColor(val) {
  ...
  if (val > -0.5) return 'var(--bg-3)';
  if (val > -1.5) return '#fca5a5';
...
const PanelSectors = memo(() => {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      if (real && real.length > 0) { setSectors(real); return; }
    } catch { /* fallback */ }
    setSectors(getSectorPerformance());
  }, []);
  ...
  const getVal = (s) => {
    if (view === 'week') return s.weekPct;
    if (view === 'month') return s.monthPct;
    return s.changePct;
  };
        ...
        <div style={{ display: 'flex', gap: 3 }}>
          {['day', 'week', 'month'].map(v => (
            <button
              ...
              onClick={() => setView(v)}
              style={{ padding: '2px 8px', fontSize: 9, textTransform: 'capitalize' }}
            >{v}</button>
          ))}
        </div>
          ...
          {sorted.map(s => {
            ...
            return (
              <div key={s.symbol} style={{
                ...
                <div style={{ fontSize: 9, fontWeight: 600, color: getTextColor(val), textAlign: 'center' }}>
                  {s.name}
                </div>
                <div style={{ fontSize: 8, color: getTextColor(val), opacity: 0.7 }}>
                  {s.symbol}
                </div>
                <div style={{ fontSize: 13, fontWeight: 700, color: getTextColor(val), fontFamily: 'JetBrains Mono, monospace' }}>
                  {val > 0 ? '+' : ''}{val.toFixed(2)}%
                </div>
              </div>
            );
