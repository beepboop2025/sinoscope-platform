import { memo, useState, useMemo, type ReactElement, type CSSProperties, type ComponentType } from 'react';
import { Radio, TrendingUp, TrendingDown, Minus, AlertTriangle, ChevronDown, ChevronUp, type LucideProps } from 'lucide-react';
...
interface Signal {
  ...
  techScore: number;
  pricePrediction?: { direction: string; confidence: number };
}
...
const strengthBadge: Record<string, { bg: string; fontWeight: number }> = {
  strong: { bg: 'rgba(255,255,255,0.1)', fontWeight: 700 },
  moderate: { bg: 'transparent', fontWeight: 500 },
  neutral: { bg: 'transparent', fontWeight: 400 },
};

const PanelSignals = memo(function PanelSignals({ mlState }: PanelSignalsProps): ReactElement {
...
function ScoreBar({ score, style }: ScoreBarProps): ReactElement {
