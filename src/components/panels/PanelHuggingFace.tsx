...
function classifyError(err: unknown): ErrorInfo {
  ...
  return { type: 'unknown', label: 'Load failed', detail: (err as Error)?.message || 'An unexpected error occurred.', icon: AlertTriangle, color: 'var(--text-3)' };
}

const PanelHuggingFace = memo((): ReactElement => {
