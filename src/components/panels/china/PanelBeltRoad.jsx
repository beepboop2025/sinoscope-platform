/**
 * PanelBeltRoad - Belt and Road Initiative Visualization
 * Interactive SVG map showing BRI corridors and participating countries
 */
...
// Corridor routes (simplified path coordinates)
...
export default function PanelBeltRoad() {
  const [selectedCorridor, setSelectedCorridor] = useState(null);
  const [hoveredCountry, setHoveredCountry] = useState(null);

  const totalProjects = REGIONS.reduce((sum, r) => sum + (r.projects || 0), 0);
  const totalInvestment = BRI_CORRIDORS.reduce((sum, c) => {
  ...
  return (
    <div className="panel-content" style={{ padding: 16 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, marginBottom: 12 }}>
        ...
        <div style={{ padding: 10, background: 'var(--surface-2)', borderRadius: 8, textAlign: 'center' }}>
          <div style={{ fontSize: 18, fontWeight: 600, color: 'var(--magenta)' }}>{BRI_CORRIDORS.length}</div>
        ...
        <svg viewBox="0 0 100 100" style={{ width: '100%', height: '100%' }}>
          ...
          {CORRIDOR_PATHS.map((corridor, idx) => (
            <path
              ...
              strokeWidth={corridor.width * 0.3}
              opacity={selectedCorridor && selectedCorridor.name !== corridor.name ? 0.2 : 0.8}
          ...
          {REGIONS.map((country) => (
            ...
            >
              <circle
                cx={country.x}
                cy={country.y}
                r={country.type === 'hub' ? 4 : country.flagship ? 3.5 : 2 + (country.projects || 0) / 50}
                fill={getCountryColor(country)}
                stroke={hoveredCountry?.id === country.id ? 'white' : 'none'}
                strokeWidth="0.5"
              />
              {country.flagship && (
                <circle
        ...
        {hoveredCountry && (
          ...
          >
            ...
            {hoveredCountry.projects > 0 && (
              <div style={{ color: 'var(--text-3)' }}>{hoveredCountry.projects} projects</div>
            )}
            {hoveredCountry.flagship && (
              <div style={{ color: 'var(--amber)', fontSize: 9 }}>Flagship Project</div>
            )}
          </div>
        )}
      </div>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 12, fontSize: 9, flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          <div style={{ width: 8, height: 8, background: 'rgba(59, 130, 246, 0.7)', borderRadius: '50%' }} />
          <span>BRI Participant</span>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        ...
        {BRI_CORRIDORS.map((corridor, idx) => (
          <div
            key={idx}
            onClick={() => setSelectedCorridor(selectedCorridor?.name === corridor.name ? null : corridor)}
            style={{
              padding: '8px 10px',
              background: selectedCorridor?.name === corridor.name ? 'var(--surface-2)' : 'var(--surface-1)',
              borderRadius: 6,
              border: `1px solid ${selectedCorridor?.name === corridor.name ? 'var(--primary)' : 'var(--divider)'}`,
              cursor: 'pointer',
              transition: 'all 0.2s',
            }}
          >
