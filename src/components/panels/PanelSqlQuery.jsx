...
const SchemaDrawer = ({ schema, onInsert }) => (
...
const SaveQueryDialog = ({ sql, onSave, onCancel }) => {
  ...
  return (
    <div style={{ display: 'flex', gap: 4, alignItems: 'center', padding: '4px 0' }}>
      <input
        ...
        onKeyDown={e => { if (e.key === 'Enter' && label.trim()) onSave(label.trim(), sql); if (e.key === 'Escape') onCancel(); }}
...
const PanelSqlQuery = memo(({ data }) => {
  ...
  const [sql, setSql] = useState('SELECT * FROM all_assets ORDER BY changePct DESC LIMIT 10');
  ...
  const handleSaveQuery = useCallback((label, querySql) => {
    saveQuery(label, querySql);
    setShowSaveDialog(false);
  }, [saveQuery]);

  const tableBadges = Object.entries(tableInfo);
                      ...
                      return (
                        <td
                          ...
                          style={{
                            ...
                            color: isChangePct ? (val > 0 ? 'var(--green)' : val < 0 ? 'var(--red)' : undefined) : undefined,
                          }}
                        >
                          {isNum
                            ? (isChangePct ? `${(Number(val) || 0).toFixed(2)}%` : Number.isInteger(val) ? val.toLocaleString() : (Number(val) || 0).toFixed(2))
                            : String(val ?? '')}
                        </td>
                      );
                    })}
