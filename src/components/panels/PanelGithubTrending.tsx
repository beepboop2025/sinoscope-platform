...
const LANG_COLORS: Record<string, string> = {
  ...
  'C#': '#178600', 'C++': '#f34b7d', Rust: '#dea584',
  Go: '#00ADD8', Java: '#b07219', 'Jupyter Notebook': '#DA5B0B', R: '#198CE7',
};
...
const PanelGithubTrending = memo((): ReactElement => {
    ...
    <PanelChrome title="GitHub Finance" icon={Github} iconColor="var(--text-1)">
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6, height: '100%', minHeight: 0 }}>
        <div style={{ display: 'flex', gap: 3, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          {languages.map(lang => (
            <button key={lang} className={filter === lang ? 'btn-primary' : 'btn-ghost'} onClick={() => setFilter(lang)} style={{ padding: '2px 7px', fontSize: 9 }}>
              <span style={{ display: 'inline-block', width: 6, height: 6, borderRadius: '50%', background: LANG_COLORS[lang] || 'var(--text-4)', marginRight: 3 }} />
              {lang}
            </button>
          ))}
        ...
        <div style={{ flex: 1, minHeight: 0, overflow: 'auto' }}>
          {loading && repos.length === 0 ? (
            ...
            filtered.map(repo => (
              <div key={repo.id} style={{ padding: '6px 8px', borderBottom: '1px solid var(--border-1)', display: 'flex', flexDirection: 'column', gap: 3 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <Code size={11} style={{ color: LANG_COLORS[repo.language] || 'var(--text-3)', flexShrink: 0 }} />
                  <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--cyan)', fontFamily: 'JetBrains Mono, monospace' }}>{repo.name}</span>
                  {repo.url && (
                ...
                <div style={{ fontSize: 10, color: 'var(--text-3)', lineHeight: 1.3 }}>{repo.description}</div>
                <div style={{ display: 'flex', gap: 8, fontSize: 9, color: 'var(--text-4)', alignItems: 'center' }}>
                  <span style={{ display: 'flex', alignItems: 'center', gap: 2, color: 'var(--yellow)' }}><Star size={9} /> {formatNum(repo.stars)}</span>
                  <span style={{ display: 'flex', alignItems: 'center', gap: 2 }}><GitFork size={9} /> {formatNum(repo.forks)}</span>
                  <span style={{ display: 'inline-block', width: 6, height: 6, borderRadius: '50%', background: LANG_COLORS[repo.language] || 'var(--text-4)' }} />
                ...
                {repo.topics.length > 0 && (
                  <div style={{ display: 'flex', gap: 3, flexWrap: 'wrap' }}>
                    {repo.topics.map(t => (
                      <span key={t} className="badge" style={{ background: 'var(--bg-3)', color: 'var(--purple)', fontSize: 8, padding: '1px 5px' }}>{t}</span>
                    ))}
                  </div>
                )}
              </div>
        ...
        <div style={{ fontSize: 9, color: 'var(--text-4)', padding: '2px 0' }}>{filtered.length} repos · sorted by stars</div>
      </div>
    </PanelChrome>
  );
});
