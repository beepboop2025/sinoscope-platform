...
import { Briefcase, Plus, Trash2, ChevronDown, TrendingUp, TrendingDown } from 'lucide-react';
...
const AddHoldingForm = ({ onAdd, onCancel }) => {
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: 'flex', gap: 4, alignItems: 'center', padding: '4px 0', flexWrap: 'wrap' }}>
      <input value={symbol} onChange={e => setSymbol(e.target.value)} placeholder="Symbol" style={{ ...inputStyle, width: 60 }} />
      <select value={type} onChange={e => setType(e.target.value)} style={{ ...inputStyle, width: 60 }}>
        <option value="stock">Stock</option>
        <option value="crypto">Crypto</option>
      </select>
      <input value={qty} onChange={e => setQty(e.target.value)} placeholder="Qty" type="number" step="any" style={{ ...inputStyle, width: 50 }} />
      <input value={cost} onChange={e => setCost(e.target.value)} placeholder="Avg Cost" type="number" step="any" style={{ ...inputStyle, width: 70 }} />
...
const PanelPortfolio = memo(({ data: marketData }) => {
  ...
  const totals = calculateTotalValue(holdings, marketData);
  const allocation = calculateAllocation(holdings, marketData);
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6, height: '100%', minHeight: 0 }}>
        ...
        {showNewPf && (
          <div style={{ display: 'flex', gap: 4 }}>
            <input
              autoFocus value={newPfName} onChange={e => setNewPfName(e.target.value)}
              onKeyDown={e => { if (e.key === 'Enter') handleCreatePortfolio(); if (e.key === 'Escape') setShowNewPf(false); }}
            ...
            <table className="dense-table">
              ...
              <tbody>
                {holdings.map(h => {
                  ...
                  const { currentValue, pnl, pnlPct } = calculateHoldingPnL(h, price);
                  return (
                    <tr key={h.id}>
                      <td>
...
function getHoldingPrice(holding, marketData) {
