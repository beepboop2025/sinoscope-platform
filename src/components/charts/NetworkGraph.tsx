import { memo, useMemo, useState, useCallback, type ReactElement } from 'react';
...
interface PositionedLink extends NetworkLink {
  sx: number | undefined;
  sy: number | undefined;
  tx: number | undefined;
  ty: number | undefined;
...
const NetworkGraph = memo(({ nodes = [], links = [], width = 400, height = 300, nodeRadius = 18, onNodeClick }: NetworkGraphProps): ReactElement => {
  const [hoveredNode, setHoveredNode] = useState<string | null>(null);

  const positioned = useMemo((): { nodes: PositionedNode[]; links: PositionedLink[] } => {
    if (nodes.length === 0) return { nodes: [], links: [] };
    const cx = width / 2, cy = height / 2;
    const r = Math.min(cx, cy) - nodeRadius - 20;
    const posNodes: PositionedNode[] = nodes.map((n, i) => {
      const angle = (2 * Math.PI * i) / nodes.length - Math.PI / 2;
      return { ...n, x: cx + r * Math.cos(angle), y: cy + r * Math.sin(angle) };
    });
    const posLinks: PositionedLink[] = links.map(l => {
  ...
  const handleNodeEnter = useCallback((id: string): void => setHoveredNode(id), []);
  const handleNodeLeave = useCallback((): void => setHoveredNode(null), []);
  const handleNodeClick = useCallback((node: PositionedNode): void => {
    if (onNodeClick) onNodeClick(node);
  }, [onNodeClick]);

  const isLinkConnected = (l: PositionedLink): boolean => {
    if (!connectedIds) return true;
    return connectedIds.has(l.source) && connectedIds.has(l.target);
  };

  const isNodeConnected = (id: string): boolean => {
    if (!connectedIds) return true;
    ...
    <svg width="100%" height="100%" viewBox={`0 0 ${width} ${height}`} style={{ minHeight: height }}>
      {positioned.links.map((l, i) => (
        <line
          ...
          stroke={(l.value ?? 0) > 0 ? 'var(--green)' : 'var(--red)'}
