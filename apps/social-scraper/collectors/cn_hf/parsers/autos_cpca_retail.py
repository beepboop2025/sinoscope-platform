"""CN-HF parser: CPCA passenger vehicle retail sales (autos_cpca_retail).

The China Passenger Car Association (CPCA / 乘联会) publishes monthly passenger
vehicle retail sales via website articles, PDF reports and its WeChat public
...
SOURCE: dict[str, Any] = {
    "key": "autos_cpca_retail",
    "name_zh": "乘联会乘用车零售销量",
    "name_en": "CPCA China Passenger Vehicle Retail Sales",
    ...
    "difficulty": "hard",
    "note": (
        "Monthly passenger-vehicle retail sales for China published by the China "
...
async def collect(http: httpx.AsyncClient, src: dict) -> list[dict]:
    ...
    logger.warning(
        "[%s] CPCA retail sales source is a TODO stub: no stable public "
