import { memo, useState, useEffect, useCallback, type ReactElement, type CSSProperties, type KeyboardEvent, type MouseEvent, type ChangeEvent } from 'react';
...
const ApiKeyManager = memo(({ isOpen, onClose }: ApiKeyManagerProps): ReactElement | null => {
  const [keys, setKeys] = useState<Record<string, string>>(() => storageRead<Record<string, string>>(STORAGE_KEY, {}));
  ...
  const handleSave = useCallback((providerId: string): void => {
  ...
  const handleDelete = useCallback((providerId: string): void => {
    setKeys(prev => {
      const next = { ...prev };
              ...
              <div key={p.id} style={{
                ...
                {isEditing && (
                  <div style={{ display: 'flex', gap: 4, marginTop: 6 }}>
                    <input
                      autoFocus value={inputVal} onChange={(e: ChangeEvent<HTMLInputElement>) => setInputVal(e.target.value)}
                      onKeyDown={(e: KeyboardEvent<HTMLInputElement>) => { if (e.key === 'Enter') handleSave(p.id); if (e.key === 'Escape') setEditing(null); }}
